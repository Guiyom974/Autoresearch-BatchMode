...
if not exist ".env" (
    ...
    echo     2. Edit .env and set: POE_API_KEY=sk_...
    echo     3. Get key from: https://poe.com/api/keys
    echo.
    pause
    exit /b 1
)

REM Check POE_API_KEY
for /f "tokens=2 delims==" %%A in ('findstr "^POE_API_KEY" .env') do set POE_API_KEY=%%A
if "!POE_API_KEY!"=="" (
    echo [!] POE_API_KEY not set in .env
    pause
    exit /b 1
)

echo [+] POE_API_KEY found
